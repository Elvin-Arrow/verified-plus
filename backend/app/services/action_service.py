...
BE-20: every method body runs under `store._lock` (an RLock, docs/design.md
§3/§6.4) -- coordinator actions are comparatively rare/human-paced next to
concurrent intake submissions, so simple whole-method locking (even though
split_out/rescue's `matching_service.rerun` call happens to include an LLM
call while held) is an acceptable simplification at this scope; only
intake_service.submit's hot path gets the finer-grained lock/unlock
treatment NFR-101 calls for.
"""
