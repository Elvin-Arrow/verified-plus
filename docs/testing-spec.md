...
- **`standalone → pending_addition`** (`_attach_to_event`'s FR-304b branch, FR-205 step 3): a request matching an already-`verified` (or `dispatched`) Event attaches as `pending_addition`, NOT `in_candidate_event` — this needs its own dedicated test distinct from the two above, and the core integration flow in §6.1 must include a request arriving *after* an Event is already verified, not only requests that build the Event before verification ever happens.
- **Authority selection among multiple geometrically-valid matches** (FR-205 step 2): a request that matches two Events that both survive the geometric filter — one `candidate`, one `verified` — must join the `verified` one (`pending_addition`), not the `candidate` one. A separate case with `verified` vs. `dispatched` on the table, asserting `dispatched` wins. This is distinct from, and in addition to, the Finding-13 regression in §4.3, which only tests the geometric filter's ordering relative to authority selection — not authority selection's own correctness when the filter doesn't exclude anything.
...
- **The three "retreat to `standalone`" paths** — each is a distinct code path and needs its own test, not one generic "reverts to standalone" case: `in_candidate_event → standalone` via `split_out` (assert `verified` stays `false`, unaffected since it was never `true`) or via `dismiss_cluster` (assert `device_flag` is untouched on every member's device, per FR-507); `in_verified_event → standalone` via `split_out` on an already-verified Event's member (assert `verified` resets to `false` — see `docs/data-model.md` §3.3's explicit correction on this point, the case most likely to get implemented backwards); `pending_addition → standalone` via the *dissolution* path only (§4.3) — there is no direct coordinator action that reverts a `pending_addition` member on its own, so this transition is exercised exclusively through the dissolution tests, not a standalone unit test.
...
- `quarantined → standalone` (`rescue`) — assert `verified` resets to `false` and matching re-runs (per `docs/design.md` §4.5b) rather than restoring stale pre-quarantine state. Note `rescue` itself does no detaching (the request was already removed from any Event's membership lists during the quarantine sweep that got it here, per `docs/design.md` §4.4) — it only flips `status` and re-triggers matching.
...
- **Invariant 1** ("no Event with ≤1 active member exists at rest"): after every mutating action that can reduce an Event's membership (`split_out`, `reject_and_flag_device`, `dismiss_cluster` — note `rescue` is NOT in this list: by the time `rescue` runs, the request was already detached from any Event during the quarantine sweep that got it there, per `docs/design.md` §4.4, so `rescue` itself never touches `member_request_ids`), assert `len(event.member_request_ids) >= 2` for every Event still present in the store — including a **specific regression test** that creates a 3-member Event, rejects two members belonging to different devices in two separate `reject_and_flag_device` calls, and asserts the Event is gone (not silently sitting at 1 member) after the second call.
...
### 4.5 Calibration buffer tests (FR-604, `record_urgency_override`/`record_duplicate_correction`)

Not covered by §4.1–4.4 above, and easy to miss entirely since neither buffer has a dedicated status field to assert against — these need their own explicit tests:

- **Eviction at N=5**: submit 6 sequential `override-urgency` calls on different requests; assert `store.urgency_calibration_buffer` has exactly 5 entries afterward and the *first* call's entry is the one evicted (FIFO), not an arbitrary one. Same test shape for `match_calibration_buffer` via 6 sequential `split_out` calls.
- **`record_duplicate_correction` population**: assert it's actually called — i.e. the buffer grows — from both trigger points named in `docs/design.md` §4.7's comment: `split_out` (§4.5b) and `dismiss_cluster` (§4.6). A test per trigger, not one combined test, since these are two independent call sites that could each be individually forgotten.
- **Buffer contents reach the prompt**: an integration-level test (not unit) asserting that when the calibration buffer is non-empty, the next `complete()` call's rendered prompt actually contains the buffered text (via the LLM double capturing its input) — this is what would catch `prompts.py` silently failing to read the buffer even though the buffer itself is correctly populated and evicted.
...
- **Two-tier queue interaction — urgency is primary, device count only breaks ties**: the lexicographic rule (`docs/spec.md` FR-401, `docs/data-model.md` §4) is `(max_urgency_score, distinct_device_count)`, urgency first — this needs two separate assertions, not one, since a single test can't distinguish "urgency-primary" from "device-count-primary" by accident:
  1. A 1-device cluster at urgency 5 sorts **ahead of** a 3-device cluster at urgency 3 — proving urgency wins even against a much larger corroboration count (this is the case that would fail under the old, removed multiplicative formula, and is the actual regression guard for `docs/spec.md` §10 finding 4).
  2. Two clusters tied at urgency 4 — the one with 3 distinct devices sorts ahead of the one with 1 — proving device count is a real, functioning tie-breaker and not dead code once urgency is controlled for.
- **Pending-addition arrival on a live Event**: build and verify a 2-member Event first, *then* submit a 3rd request that matches it — assert the 3rd request attaches as `pending_addition` (not auto-promoted into the Dispatch Queue, FR-304b), the Event's `member_count`/centroid are unaffected until `approve_pending` runs, and `approve_pending` then promotes it correctly with `verified=true`. This is the scenario §6.1's original cluster-building test omitted entirely (it only ever built clusters *before* verification), and is what would catch FR-304b's core protection going untested.
- **Authority selection with no geometric exclusion**: seed a `candidate` Event and a `verified` Event, both within `max_cluster_span_km` of a new request that matches both — assert it joins the `verified` one as `pending_addition`, not the `candidate` one as a direct member. Repeat with `verified` vs. `dispatched` — assert `dispatched` wins. Distinct from the Finding-13 regression test (§4.3), which only exercises authority selection *after* the geometric filter has already excluded one candidate — this test exercises authority selection on its own, with nothing excluded.
...
| Scenario | Traces to |
|---|---|
| A genuine mass-casualty event (many distinct devices, high urgency, no `null`-urgency members) reaches the top of the Intake Inbox's *sorted section* and, once verified, the top of the Dispatch Queue. (Never above the Needs Manual Triage section, per FR-401 §1 — that section is unsorted-by-formula by definition, not something any event outranks.) | FR-401, FR-403, `docs/spec.md` §10 finding 4 |
| A single-device paraphrase-spam flood does NOT reach the top of the Intake Inbox's sorted section unattended. | FR-401's device-count sort key, `docs/spec.md` §10 finding 1/9 |
...
| FR-3xx Verification/device | State-machine (§4.1–4.3), integration (§6.1, incl. the pending-addition and authority-selection cases) |
| FR-4xx Queues | Unit (§3.1 sort), acceptance (§7) |
| FR-5xx Incident Cards/actions | State-machine (§4.1–4.3), mutation (§5), acceptance (§7) |
| FR-6xx Feedback loop | Unit (§3.1 prompt snapshot), state-machine (§4.1), calibration buffer (§4.5), mutation (§5 — `record_urgency_override`) |
...
## 12. Change log — 7-document alignment pass (v0.1 → v0.2)

A Gemini review reading this document against all six prior project documents found:

1. **A backwards sort-priority test** (§6.1): the original scenario asserted a 3-device cluster with lower urgency sorts ahead of a 1-device cluster with higher urgency — exactly the multiplicative-formula bug the lexicographic sort was built to prevent. Split into two correct, distinct assertions: urgency wins even against a much larger device count, and device count only breaks an actual urgency tie.
2. **Misleading "top of the Intake Inbox" language** (§7): didn't account for the Needs Manual Triage section always rendering above the sorted list regardless of any item's urgency/device count. Reworded to specify "the sorted section."
3. **A factually wrong claim that `rescue` performs an "implicit detach"** (§4.1, §4.3): `rescue` only flips `status`; the detach already happened during the earlier quarantine sweep. Corrected in both places, and removed from Invariant 1's list of dissolution-triggering actions.
4. **Missing state transitions**: the `standalone → pending_addition` path (FR-304b) and all three "retreat to `standalone`" paths (`in_candidate_event`/`in_verified_event`/`pending_addition` → `standalone`) had no test coverage at all in v0.1. Added to §4.1, including the easy-to-get-backwards case of Split Out resetting `verified` to `false` even when the split-out member was previously verified.
5. **No coverage of authority selection on its own**: the only authority-related test was the Finding-13 regression, which exercises authority selection *after* a geometric exclusion — never on its own, with multiple valid candidates and nothing excluded. Added as a dedicated integration test in §6.1.
6. **No coverage of the pending-addition flow**: §6.1's core integration test only ever built clusters before verification, never exercising a request arriving *after* an Event is already verified — the exact scenario FR-304b exists to protect. Added.
7. **No calibration-buffer tests at all**: N=5 eviction, `record_duplicate_correction`'s two trigger points (`split_out`, `dismiss_cluster`), and whether buffered corrections actually reach the rendered LLM prompt were entirely untested. Added as new §4.5.
